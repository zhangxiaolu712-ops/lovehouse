...
internal class ChatVoiceInputController(
    context: Context,
    private val onState: (ChatVoiceInputState) -> Unit,
) : RecognitionListener {
    private val recognizer = SpeechRecognizer.createSpeechRecognizer(context.applicationContext).also {
        it.setRecognitionListener(this)
    }
    private var latest = ChatVoiceInputState()

    fun start() {
        latest = ChatVoiceInputState(listening = true)
        onState(latest)
    ...
    fun stop() {
        if (latest.listening) runCatching { recognizer.stopListening() }
    }

    fun dismiss() {
        runCatching { recognizer.cancel() }
        latest = ChatVoiceInputState()
        onState(latest)
    }
    ...
    private fun update(
        ...
        latest = ChatVoiceInputState(listening, processing, transcript, finished, error)
        onState(latest)
    }
}
