...
test('real Codex 0.146 JSONL shape maps text, safe tool events, usage and unavailable reasoning', async () => {
  ...
  assert.deepEqual(emitted.map(item => item.event), [
    'tool_call', 'tool_result', 'usage', 'reasoning_status',
  ])
  assert.deepEqual(emitted[0].data, {
    call_id: 'item_1', tool_type: 'command', name: 'shell', status: 'running',
  })
  assert.match(emitted[1].data.summary, /^Command completed/)
  ...
  assert.equal(emitted[2].data.actual_input_tokens, 32062)
  assert.equal(emitted[2].data.actual_output_tokens, 52)
  assert.equal(emitted[2].data.total_tokens, 32114)
  assert.equal(emitted[2].data.usage_source, 'codex_cli')
  assert.deepEqual(emitted[3].data, {
    available: false, status: 'unavailable', summary: null, source: 'codex_cli',
  })
  assert.deepEqual(calls[0].args.slice(0, 3), ['exec', '--json', '--sandbox'])
...
test('user-visible Codex reasoning item is passed as a bounded redacted summary, never fabricated', async () => {
  ...
  assert.equal(reasoning.summary, 'Checking Bearer [redacted] before answering.')
  assert.equal(emitted.filter(item => item.event === 'reasoning_status').length, 1)
})
...
test('confirmed missing resume recovers with bounded context but keeps caller thread identity outside adapter', async () => {
  ...
  assert.deepEqual(calls[0].args.slice(0, 3), ['exec', 'resume', 'aaaaaaaa-bbbb-4ccc-8ddd-eeeeeeeeeeee'])
  assert.deepEqual(calls[1].args.slice(0, 2), ['exec', '--json'])
  assert.equal(result.sessionId, SESSION_ID)
