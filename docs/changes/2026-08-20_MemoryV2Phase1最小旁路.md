...
- Starter Pack 不使用 importance 排序：专用只读 RPC 从全部当前可见事实中只取 current revision、active、未 superseded、actor private + approved Shared，并排除 inactive/fulfilled/cancelled commitment 与 revoked Shared。有效承诺最多 4，最近 revision 最多 8，盲盒从排除前两类后的完整合格池随机抽最多 3；没有 45 条或 recent-pool 预截断。三类去重，总数软上限 15，1600 estimated-token 为最终硬预算，长正文只给 bounded excerpt，不展开 source。
