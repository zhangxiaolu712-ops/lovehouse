...
- 日期：2026-08-20
- 执行者：Codex
- 状态：Approved Shared embedding delta 已上线；adapter 已完成；生产 backfill 因单条 timeout 停在 46/58，semantic 验收未完成
...
- Backfill 目标 58，成功 46、跳过 0、失败 1；当前 GPT 32/32、Shared 2/2、Claude 12/24 已有 `qwen3-embedding:4b` embedding。失败项是 current Claude revision，正文 1910 字符，30 秒超时后脚本按设计停止。
...
- 真正 semantic query、断连 lexical fallback 与恢复 semantic 尚未执行：backfill 首错即停边界优先。

## 已知风险或未完成事项

- Claude 仍有 12 个 current revision 未 embedding，因此 Phase 2E 尚未达到完整 semantic 验收。
...
1. 经复审后重新运行同一 backfill；已存在的 46 条会跳过，从失败 revision 继续。
2. Backfill 完成后再执行真实 GPT/Claude/Shared semantic query、断连 fallback 与自动恢复验收。
3. 不自行切 MCP/Chat、部署 Bridge、进入 Phase 2C 或清理 V1。
