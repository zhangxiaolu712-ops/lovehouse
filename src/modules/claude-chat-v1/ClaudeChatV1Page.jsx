...
import { deriveCurrentTurnUsage } from '../codex-chat-v1/codexChatV1State'
import '../codex-chat-v1/codexChatV1.css'
import { streamClaudeV1 } from './claudeChatV1Service'
import {
  boundClaudeV1History,
  getClaudeV1Identity,
  loadClaudeV1History,
  saveClaudeV1History,
} from './claudeChatV1State'
...
export default function ClaudeChatV1Page() {
  const identity = useRef(getClaudeV1Identity())
  const [messages, setMessages] = useState(() => loadClaudeV1History())
  ...
  useEffect(() => { saveClaudeV1History(messages) }, [messages])

  async function send(event) {
    event?.preventDefault()
    const message = input.trim()
    if (!message || loading) return
    setMessages(current => boundClaudeV1History([...current, { role: 'user', content: message }]))
    setInput('')
    setStream('')
    setLoading(true)
    ...
    try {
      ...
      if (result.ok && result.text) {
        setMessages(current => boundClaudeV1History([
          ...current,
  ...
  return (
    <main className="codex-v1-page">
      <header className="codex-v1-header">
        <div>
          <Link to="/">← 返回</Link>
          <p>UNIFIED CHAT MAINLINE · CLAUDE ADAPTER V1</p>
      ...
      <section className="codex-v1-facts" aria-label="Chat runtime facts">
        <div><span>Persona</span><strong>小克</strong></div>
        <div><span>Runtime</span><strong>{runtime.runtime_type || 'claude_cli'}</strong></div>
        <div><span>Adapter</span><strong>{runtime.adapter_id || 'claude-cli-v1'}</strong></div>
        <div className="is-wide"><span>LoveHouse Thread</span><code>{identity.current.threadId}</code></div>
        <div><span>Scene</span><strong>casual · text</strong></div>
      </section>

      <div className="codex-v1-grid">
        <section className="codex-v1-chat-panel">
          <div className="codex-v1-messages" aria-live="polite">
            {!messages.length && !loading && <p className="codex-v1-empty">发一条消息，验证 Claude 接入同一条 Chat 主干。</p>}
            {messages.map((message, index) => (
              <article key={`${message.role}-${index}`} className={`is-${message.role}`}>
                <small>{message.role === 'user' ? '小婷' : '小克'}</small>
        ...
        <aside className="codex-v1-observability">
          ...
          <section>
            ...
            {context?.reasoning && (
              <small>
                reasoning 由 Claude 原生 session 管理；LoveHouse 不重复注入，也不自建 compaction。
