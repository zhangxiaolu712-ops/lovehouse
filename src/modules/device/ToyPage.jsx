...
async function sendCommand(intensity, pattern) {
  const { error } = await supabase
    .from('toy_commands')
    .insert({ intensity, pattern: pattern || null })
  return !error
}
...
export default function ToyPage() {
  ...
  async function send(val, pat) {
    setSending(true)
    const ok = await sendCommand(val, pat)
    if (ok) {
      setIntensity(val)
  ...
  return (
    <div>
      ...
      <div className="card toy-main" style={{ textAlign: 'center', padding: '32px 24px', marginBottom: 16 }}>
        ...
        <div className="toy-slider-wrap">
          <input
            ...
            onMouseUp={() => send(intensity)}
            onTouchEnd={() => send(intensity)}
        ...
        {/* 停止按钮 */}
        <button className="toy-stop-btn" onClick={stop} disabled={sending}>
          ⏹ 停止
        </button>
      </div>
      ...
      <div className="card" style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.8 }}>
        ...
        <p>1. 电脑下载并运行 <code>python toy_controller.py listen</code></p>
        <p>2. 手机保持司沃康 app 在前台、屏幕常亮</p>
        <p>3. 在这里滑滑条，或者跟小克聊天说「轻一点」「强一点」</p>
        <p>4. 指令通过云端下发，电脑自动执行</p>
      </div>
    </div>
  )
}
