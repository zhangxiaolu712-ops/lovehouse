  ...
  const result = await streamCodexV1({
    ...
    fetchImpl: async (url, options) => {
      ...
      return new Response([
        ...
        'event: reasoning_status\ndata: {"available":false,"status":"unavailable","summary":null}',
        'event: tool_call\ndata: {"call_id":"item-1","name":"shell","status":"running"}',
        'event: tool_result\ndata: {"call_id":"item-1","name":"shell","status":"success","summary":"Command completed"}',
        ...
        'event: context_breakdown\ndata: {"recent_chat":{"enabled":true},"memory":{"enabled":false},"worldbook":{"enabled":false},"persona":{"enabled":false},"current_message":{"enabled":true},"estimated_tokens":2}',
        'event: text_delta\ndata: {"delta":"hel"}',
        'event: text_delta\ndata: {"delta":"lo"}',
        'event: message_end\ndata: {"ok":true}',
        '',
