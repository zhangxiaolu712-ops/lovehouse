...
- chatService：localStorage管理+标准Anthropic流式API（兼容CC-P）
- 点击头像弹出个人面板（模型/轮数/起始时间），可配置API连接
- LineIcon新增 back/send/close 图标
